...
def diagnose(db: Session = Depends(get_db),
    ...
    channels = []
    li_pause = _ap.get_reauth(db, "linkedin")
    ...
    if li_pause:
        channels.append({"name": "LinkedIn", "ok": False,
                         "detail": "PAUSED - token died mid-delivery; queued posts are waiting",
                         "fix": li_pause.get("reason", "Settings -> One-click Connect -> LinkedIn")})
    else:
    ...
    gbp_pause = _ap.get_reauth(db, "google_business")
    ...
    fix = None
    if gbp_pause and not gbp_ok:
        gbp_det = "PAUSED - token died mid-delivery; queued posts are waiting"
        fix = gbp_pause.get("reason")
    elif not gbp_ok:
        low = gbp_det.lower()
        if "invalid_client" in low or "oauth client was not found" in low:
            ...
            fix = ("The Google refresh token was revoked/expired - reconnect via Settings -> "
                   "One-click Connect -> Google Business. If it dies again in ~a week, your "
                   "Google OAuth consent screen is in 'Testing' mode (tokens expire after 7 "
                   "days) - set it to 'In production' in Google Cloud -> OAuth consent screen.")
        elif "not configured" in low:
            fix = "Settings -> Google Business Profile: connect + pick your location."
    elif gbp_pause:
        _ap.clear_reauth(db, "gbp")   # live check passed - lift a stale pause
